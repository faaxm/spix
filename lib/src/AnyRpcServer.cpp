...
: m_pimpl(new AnyRpcServerPimpl())
{
    ...
    utils::AddFunctionToAnyRpc<void(std::string, int)>(methodManager, "mouseClickWithButton",
        "Click on the object at the given path with the given mouse button | mouseClickWithButton(string path, int mouseButton)",
    ...
    utils::AddFunctionToAnyRpc<void(std::string, std::string, std::string)>(methodManager, "setStringProperty",
        "Set the string value of the given property | setStringProperty(string path, string property, string new_value)",
    ...
    utils::AddFunctionToAnyRpc<std::vector<double>(std::string)>(methodManager, "getBoundingBox",
        "Return the bounding box of an item in screen coordinates | getBoundingBox(string path) : (doubles) [topLeft.x, topLeft.y , width, height]", [this](std::string path) {
    ...
    utils::AddFunctionToAnyRpc<void(std::string, std::string)>(methodManager, "takeScreenshot",
        "Take a screenshot of the object and save it as a file | takeScreenshot(string pathToTargetedItem, string filePath)",
