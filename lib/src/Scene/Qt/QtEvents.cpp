...
QQuickWindow *getWindowAndPositionForItem(Item *item, Point relToItemPos,
                                          QPointF &windowPos) {
...
void sendQtKeyEvent(Item *item, bool press, int keyCode,
                    QtEvents::KeyModifier mod) {
  auto qtitem = dynamic_cast<QtItem *>(item);
  if (!qtitem)
    return;
  ...
  auto keyEvent = new QKeyEvent(press ? QEvent::KeyPress : QEvent::KeyRelease,
                                keyCode, qtmod);
...
void QtEvents::mouseDown(Item *item, Point loc, MouseButton button) {
  ...
  QMouseEvent *event =
      new QMouseEvent(QEvent::MouseButtonPress, windowLoc, eventCausingButton,
                      activeButtons, Qt::KeyboardModifier::NoModifier);
...
void QtEvents::mouseUp(Item *item, Point loc, MouseButton button) {
  ...
  QMouseEvent *event =
      new QMouseEvent(QEvent::MouseButtonRelease, windowLoc, eventCausingButton,
                      activeButtons, Qt::KeyboardModifier::NoModifier);
  QGuiApplication::postEvent(window, event);
}
...
void QtEvents::mouseMove(Item *item, Point loc) {
  ...
  QMouseEvent *mouseMoveEvent =
      new QMouseEvent(QEvent::MouseMove, windowLoc, Qt::MouseButton::NoButton,
                      activeButtons, Qt::KeyboardModifier::NoModifier);
...
void QtEvents::stringInput(Item *item, const std::string &text) {
  ...
  auto keyDownEvent = new QKeyEvent(QEvent::KeyPress, 0 /* key id */,
...
void QtEvents::extMouseDrop(Item *item, Point loc, PasteboardContent &content) {
  ...
  auto enter = new QDragEnterEvent(
      windowLocInt, Qt::CopyAction | Qt::MoveAction | Qt::LinkAction, mimeData,
  ...
  auto drop = new QDropEvent(
      windowLoc, Qt::CopyAction | Qt::MoveAction | Qt::LinkAction, mimeData,
      Qt::MouseButton::NoButton, Qt::KeyboardModifier::NoModifier);
  QGuiApplication::postEvent(window, drop);
}

void QtEvents::quit() { QGuiApplication::quit(); }
