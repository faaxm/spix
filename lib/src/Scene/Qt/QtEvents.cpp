...
void QtEvents::mouseDown(Item* item, Point loc, MouseButton button)
{
    ...
    QMouseEvent* event = new QMouseEvent(QEvent::MouseButtonPress, windowLoc, eventCausingButton, activeButtons, Qt::KeyboardModifier::NoModifier);
...
void QtEvents::mouseUp(Item* item, Point loc, MouseButton button)
{
    ...
    QMouseEvent* event = new QMouseEvent(QEvent::MouseButtonRelease, windowLoc, eventCausingButton, activeButtons, Qt::KeyboardModifier::NoModifier);
    QGuiApplication::postEvent(window, event);
}
...
void QtEvents::mouseMove(Item* item, Point loc)
{
    ...
    QMouseEvent* mouseMoveEvent
        = new QMouseEvent(QEvent::MouseMove, windowLoc, Qt::MouseButton::NoButton, activeButtons, Qt::KeyboardModifier::NoModifier);
    QGuiApplication::postEvent(window, mouseMoveEvent);

    // Wiggle the cursor a bit. This is needed to correctly recognize drag events
    windowLoc.rx() += 1;
    mouseMoveEvent = new QMouseEvent(QEvent::MouseMove, windowLoc, Qt::MouseButton::NoButton, activeButtons, Qt::KeyboardModifier::NoModifier);
    QGuiApplication::postEvent(window, mouseMoveEvent);
}
...
void QtEvents::stringInput(Item* item, const std::string& text)
{
    ...
    auto keyDownEvent
        = new QKeyEvent(QEvent::KeyPress, 0 /* key id */, Qt::KeyboardModifier::NoModifier, QString::fromStdString(text));
...
void QtEvents::extMouseDrop(Item* item, Point loc, PasteboardContent& content)
{
    ...
    auto enter = new QDragEnterEvent(
        windowLocInt, Qt::CopyAction | Qt::MoveAction | Qt::LinkAction, mimeData, Qt::MouseButton::NoButton, Qt::KeyboardModifier::NoModifier);
    ...
    auto drop = new QDropEvent(
        windowLoc, Qt::CopyAction | Qt::MoveAction | Qt::LinkAction, mimeData, Qt::MouseButton::NoButton, Qt::KeyboardModifier::NoModifier);
